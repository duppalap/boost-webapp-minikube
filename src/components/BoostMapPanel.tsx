...
export default function BoostMapPanel({
  ...
  const [boostDeviceInfoOpen, setBoostDeviceInfoOpen] =
  ...
  const onClick = (event: MapEvent) => {
    if (event?.features && event?.features?.length > 0) {
      ...
      if (clusterId) {
        ...
        const newBoostDevice: BoostDeviceInterface = JSON.parse(
          feature.properties.boostDevice
        );
        if (
          selectedBoostDevice &&
          selectedBoostDevice.boostId === newBoostDevice.boostId
        ) {
          return;
        }
        updateSelectedBoostDevice(newBoostDevice);
        if (changeUtilizationSummary)
          changeUtilizationSummary(dateRange, newBoostDevice);
      }
    }
  };
